...
const groupsTemplate = distribution.util.groups;
    ...
    startNodes(() => {
      groupsTemplate(ncdcConfig).put(ncdcConfig, ncdcGroup, (e, v) => {
        ...
        groupsTemplate(dlibConfig).put(dlibConfig, dlibGroup, (e, v) => {
          ...
          groupsTemplate(tfidfConfig).put(tfidfConfig, tfidfGroup, (e, v) => {
            done();
          });
        });
      });
...
test('(25 pts) all.mr:tfidf', (done) => {
  // Map function: emit (word, docID) -> count
  let m4 = (key, value) => {
    let words = value.split(/\s+/).filter((e) => e !== '');
    let out = [];
    let wordCounts = {};
    ...
    for (let word in wordCounts) {
      const value = {};
      value[word] = {doc: key, count: wordCounts[word]};
      out.push(value);
    }
    return out;
  };
  ...
  let r4 = (key, values) => {
    let out = {};
    out[key] = {};
    let docCount = values.length;
    let totalDocs = 3;

    values.forEach(({doc, count}) => {
      let tf = count / values.reduce((sum, v) => sum + v.count, 0);
      let idf = Math.log(totalDocs / docCount);
      const tfidf = (tf * idf).toFixed(2);
      out[key][doc] = tfidf;
    });
  ...
  const doMapReduce = (cb) => {
    distribution.tfidf.store.get(null, (e, v) => {
      ...
      distribution.tfidf.mr.exec({keys: v, map: m4, reduce: r4}, (e, v) => {
        try {
          expect(v).toEqual(expect.arrayContaining(expected));
          done();
        } catch (e) {
